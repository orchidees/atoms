...
%
%
function [dist BSF] = min_distDTWBSFSub(str1, U, L, alphabet_size, compression_ratio, BSF, maxN)
    
    if (length(str1) ~= length(U))
    ...
    %dist_matrix = build_dist_table(alphabet_size);
        ...
        for i = 1:length(str1)
            ...
            elseif str1(i) < L(i)
                tmpVal = tmpVal + ((str1(i) - L(i)) .^ 2);
            end
            if (sqrt(tmpVal) / length(str1)) > (BSF(currentBSF))
                if (currentBSF == maxN)
                    tmpVal = (tmpVal + length(str1) - i);
                    break;
                else
        ...
        dist = (sqrt(tmpVal) / length(str1));
        if (i == length(str1))
            while (currentBSF <= length(BSF)) && (dist > (BSF(currentBSF)))
                currentBSF = currentBSF + 1;
            end
            BSF = [BSF(1:(currentBSF - 1)) dist BSF(currentBSF:end)];
            if length(BSF) > maxN
                BSF = BSF(1:maxN);
            end
        end
