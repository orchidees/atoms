...
%                    <esling@ircam.fr>
%
function queryValues = getSoundsQueryMultiObjStr(connecDB, queries, maxNum)
...
if (nbTQueries > 0)
    for i = 1:nbTQueries
        ...
        dShape(i, :) = tQueries{i}.value{1};
    end
    tmpResults = getTemporalQueryDTWMultiObjStr(connecDB, dShape, dName, staticQ, ceil(maxNum / 5));
    disp(tmpResults);
%    tmpResults(:, 2) = tmpResults(:, 2) ./ max(tmpResults(:, 2));
%    queryValues = sortrows(resultsCell, 2);
    queryValues = tmpResults{1};
